...
- **커밋먼트 방식(미국식)과의 대비** — 미국 LP 펀드는 좌 없이 LP마다 **임의 금액을 약정**하고(양자화 없음), 몫은 **지분율(percentage interest) = 약정액 ÷ 총 약정액**로 계산한다. 캐피탈콜·배분·의결이 전부 이 비율(pro-rata) 기준. 후속 클로징 LP는 **이퀄라이제이션(equalization)** — 기납입분+이자 정산 — 으로 기존 LP와 형평을 맞춘다. 즉 한국도 LP별 금액은 제각각이지만, 그 금액이 1좌 금액의 정수배로 양자화되고 계산 축이 좌수라는 점이 다르다.
- **LP별 출자 상한(집중도 관리)** — 법정 단일 상한은 없지만 실무상 세 방향의 상한이 작동한다: (1) **GP 측** 집중도 관리: 단일 LP 디폴트·거버넌스 지배 리스크를 피하려 통상 한 LP를 총액의 10~25% 내로 관리하고, 초과 청약 시 클로징에서 **배정 축소(scale-back)**. 앵커 LP는 예외적으로 큰 비중+특혜를 받기도 함. (2) **LP 측** 내부 규정: "한 펀드의 10~20% 초과 금지", "단일 펀드가 대체투자 배분의 일정 % 초과 금지". (3) **규제 트리거**: 미국 **ERISA 25% 테스트**(연금성 자금이 지분 25% 초과 시 펀드 자산이 plan assets로 취급되어 ERISA 수탁 규제 적용, GP가 25% 미만 유지 조항을 문서에 명시), 은행계 LP 지분·의결권 제한 등. 한국의 모태펀드 등 정책 LP는 반대로 **출자공고에서 자기 출자비율 상한**을 정하고 민간 매칭을 요구한다.
